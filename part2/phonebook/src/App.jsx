...
const App = () => {
    ...
    if (confirmDelete) {
      deletePerson(id)
        ...
        .catch((error) => {
          ...
          showNotification(
            `Failed to delete ${personToDelete.name}. Maybe it was already removed.`,
            "error"
          );
  ...
  const filteredPersons = filter
    ? persons.filter(
        (person) => person.name.toLowerCase().includes(filter.toLowerCase()) // Using includes for broader filtering
      )
    : persons;
